...
(() => {
    ...
    const createItemLink = (item, group, { disconnected = false, normal = false, defaultOrder = 0 } = {}) => {
        const wrapper = createElement("div", `flink-list-item${normal ? " flink-lazy-pending" : ""}`);
        if (normal && item.tag) {
            String(item.tag)
                .split(/\s+/)
                .filter(Boolean)
        ...
        if (normal)
            wrapper.dataset.defaultOrder = String(defaultOrder);
        ...
        anchor.append(createImage(avatar, item.name, "flink-avatar cf-friends-avatar"), createElement("div", "img-alt is-center", item.name));
    ...
    const createItemGroup = (group, { disconnected = false, normal = false, id = "", initialSort = "default" } = {}) => {
        const list = createElement("div", disconnected
        ...
        if (id)
            list.id = id;
        if (normal)
            list.dataset.initialSort = initialSort;
        ...
        orderedEntries.forEach(({ item, defaultOrder }) => fragment.append(createItemLink(item, group, { disconnected, normal, defaultOrder })));
        list.append(fragment);
        return list;
    };
    const createGroupHeading = (group, count) => {
        const heading = createElement("h2", "", `${group.class_name || ""} (${count})`);
        if (!group.title_link || !group.title_link_text)
            return heading;
    ...
    const createNormalHeading = (group, count, listId, sortMode) => {
        ...
        content.append(createElement("h2", "", `${group.class_name || ""} (${count})`), createElement("div", "flink-desc", group.descr || ""));
        ...
        sort.append(createElement("span", "", sortMode === "default" ? config.default_sort : config.random_sort));
    ...
    const bindNormalControls = (target) => {
        target.querySelectorAll(".flink-normal-sort-btn").forEach((button) => {
            ...
            button.addEventListener("click", () => {
                ...
                const orderedItems = nextMode === "random"
                    ? shuffle(items)
                    : items.sort((left, right) => Number(left.dataset.defaultOrder) -
    ...
    const createCard = (item, group) => {
        ...
        imageLink.append(createImage(`${item.topimg || ""}${group.topimg_suffix || ""}`, item.name, "flink-avatar"));
        ...
        avatar.append(createImage(`${item.avatar || ""}${group.suffix || ""}`, item.name, "flink-avatar cf-friends-avatar"), createElement("div", "img-alt is-center", item.name));
    ...
    const createError = (retry, compact = false) => {
        const status = createElement("div", `friend-links-status is-error${compact ? " is-compact" : ""}`);
        status.setAttribute("role", "alert");
        status.append(createElement("span", "", config.error));
        const button = createElement("button");
        button.type = "button";
        button.append(createElement("i", "solitude fas fa-arrows-rotate"), createElement("span", "", config.retry));
    ...
    const renderList = async (target) => {
        try {
            ...
            if (!target.isConnected || target.dataset.friendLinksLoaded === "true")
                return;
            const fragment = document.createDocumentFragment();
            let normalIndex = 0;
            data.links.forEach((group) => {
                ...
                if (group.type === "card") {
                    fragment.append(createGroupHeading(group, links.length), createElement("div", "flink-desc", group.descr || ""), createCardGroup(normalizedGroup));
                }
                    ...
                    fragment.append(createNormalHeading(group, links.length, listId, initialSort), createItemGroup(normalizedGroup, {
                        normal: true,
                        id: listId,
                        initialSort,
                    }));
                    ...
                    fragment.append(createGroupHeading(group, links.length), createElement("div", "flink-desc", group.descr || ""), createItemGroup(normalizedGroup, { disconnected: true }));
                }
            });
            ...
            [0, 1].forEach(() => {
                pairs.forEach((pair) => {
                    ...
                    pair.forEach((item) => {
                        ...
                        anchor.title = item.name || "";
                        anchor.append(createImage(`${item.avatar || ""}${data.banner_suffix || ""}`, item.name), createElement("span", "tags-group-title", item.name));
    ...
    const renderFooter = async () => {
        ...
        try {
            ...
            if (!target.isConnected)
                return;
            const links = data.links.flatMap((group) => (Array.isArray(group.link_list) ? group.link_list : []).map((item) => ({
                name: item.name,
                link: item.link,
            })));
            const fragment = document.createDocumentFragment();
    ...
    Solitude.travelling = async () => {
        try {
            ...
            if (!links.length)
                throw new Error("No friend links available");
            ...
            showToast({ text: config.error, duration: 3000, pos: "top-center" });
        }
    };
