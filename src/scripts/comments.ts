...
const fetchAggregateComments = () => {
  ...
  const cached = Solitude.saveToLocal.get(aggregateCacheKey());
  if (cached) {
...
const fetchPostCardComments = (paths: string[]) => {
  ...
  const cached = Solitude.saveToLocal.get(postCardCacheKey(signature));
  if (cached) {
...
export const initializeOtherProviders = async (enabled: CommentProvider[]) => {
  ...
  for (const provider of enabled.filter((p) => p !== 'valine')) {
    ...
    try {
      ...
      let instance: any;
      if (provider === 'twikoo')
        instance = await (window as any).twikoo.init(options);
      if (provider === 'waline')
        instance = (window as any).Waline.init(options);
      if (provider === 'artalk')
        instance = (window as any).Artalk.init({
          ...options,
