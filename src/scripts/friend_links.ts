...
(() => {
  ...
  let request: Promise<LinksData> | null = null;
  ...
  const resolveSiteUrl = (value: unknown) => {
    if (!value) return '';
  ...
  const createImage = (src: string, alt?: string, className?: string) => {
  ...
  const createTag = (item: FriendLink) => {
  ...
  const createItemLink = (
    ...
    anchor.href = disconnected
      ? 'javascript:void(0);'
      : resolveSiteUrl(item.link);
    ...
    info.append(
      createElement('span', 'flink-item-name cf-friends-name', item.name),
    );
  ...
  const createItemGroup = (
    ...
    const orderedEntries =
      initialSort === 'random' ? shuffle(entries) : entries;
  ...
  const createGroupHeading = (group: FriendGroup, count: number) => {
  ...
  const bindNormalControls = (target: HTMLElement) => {
    target
      .querySelectorAll<HTMLElement>('.flink-normal-sort-btn')
      .forEach((button) => {
    ...
    target
      .querySelectorAll<HTMLElement>('.flink-normal-scroll-controls')
      .forEach((controls) => {
        if (controls.dataset.bound === 'true') return;
        controls.dataset.bound = 'true';
        controls
          .querySelectorAll<HTMLElement>('.flink-normal-scroll-btn')
          .forEach((button) => {
            button.addEventListener('click', () => {
              const list = document.getElementById(
                controls.dataset.target || '',
              );
              if (!list) return;
  ...
  const createCard = (item: FriendLink, group: FriendGroup) => {
    ...
    imageLink.append(
      createImage(
        `${item.topimg || ''}${group.topimg_suffix || ''}`,
        item.name,
        'flink-avatar',
      ),
    );
  ...
  const createCardGroup = (group: FriendGroup) => {
    const list = createElement('div', 'site-card-group');
    const fragment = document.createDocumentFragment();
    group.link_list.forEach((item: FriendLink) =>
      fragment.append(createCard(item, group)),
    );
    list.append(fragment);
    return list;
  };
  ...
  const createError = (retry: () => void, compact = false) => {
  ...
  const renderList = async (target: HTMLElement) => {
    try {
      ...
      const fragment = document.createDocumentFragment();
      let normalIndex = 0;
      data.links.forEach((group: FriendGroup) => {
        ...
        if (group.type === 'card') {
          ...
          const initialSort =
            group.sort === 'default' || normalIndex === 0
              ? 'default'
              : 'random';
          fragment.append(
  ...
  const renderBanner = async (target: HTMLElement) => {
    try {
      const data = await load();
      if (!target.isConnected || target.dataset.friendLinksLoaded === 'true')
        return;
      const links = data.links
        .filter((group: FriendGroup) => group.type !== 'discn')
        .flatMap((group: FriendGroup) =>
          Array.isArray(group.link_list) ? group.link_list : [],
        )
        .slice(0, 30);
      const wrapper = createElement('div', 'tags-group-wrapper');
      const pairs: FriendLink[][] = [];
  ...
  const renderFooter = async () => {
    ...
    try {
      const data = await load();
      if (!target.isConnected) return;
      const links = data.links.flatMap((group: FriendGroup) =>
        (Array.isArray(group.link_list) ? group.link_list : []).map(
          (item: FriendLink) => ({
            name: item.name,
            link: item.link,
          }),
  ...
  const init = () => {
    const list = document.querySelector<HTMLElement>(
      '[data-friend-links-list]',
    );
    const banner = document.querySelector<HTMLElement>(
      '[data-friend-links-banner]',
    );
