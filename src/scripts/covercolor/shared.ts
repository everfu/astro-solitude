...
interface ColorStore {
  postcolor?: Record<string, { value: string; expiration: number }>;
}
const readStore = (): ColorStore => {
  try {
...
const writeStore = (value: ColorStore) => {
  try {
...
export const getCachedColor = (source: string) => {
  const item = readStore().postcolor?.[source];
  if (!item || (item.expiration && item.expiration <= Date.now())) return null;
  return item.value;
};
export const cacheColor = (source: string, color: string) => {
  const store = readStore();
  store.postcolor ||= {};
  store.postcolor[source] = {
    value: color,
    expiration:
      Date.now() + (Number(Solitude.config.covercolor.time) || 43200000),
  };
  writeStore(store);
};
export const getCoverSource = (music = false) => {
  ...
  const background =
    document.querySelector<HTMLElement>('#nav-music .aplayer-pic')?.style
      .backgroundImage || '';
  return /url\(["']?([^"')]+)["']?\)/.exec(background)?.[1] || '';
};
export const rgbToHex = ([r, g, b]: readonly number[], factor = 1) =>
  `#${[r, g, b]
    .map((value) =>
...
export const applyMusicColor = (value: string | null | undefined) => {
...
export const applyThemeColor = (value: string | null | undefined) => {
  const color = normalizeHex(value);
...
export const resolveColor = async (
  source: string,
  fetchColor: (source: string) => Promise<string | null>,
  music = false,
) => {
  const applyDefault = music ? applyDefaultMusicColor : applyDefaultColor;
  if (!source) return applyDefault();
