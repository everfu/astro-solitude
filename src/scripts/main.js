...
const loadFeatureModules = async () => {
    ...
    if (features.friend_links)
        requests.push(import("./friend_links"));
    if (features.keyboard)
        requests.push(import("./keyboard"));
    if (features.right_menu)
        requests.push(import("./right_menu"));
    if (features.translate)
        requests.push(import("./tw_cn"));
    if (features.music) {
    ...
    if (features.covercolor) {
        ...
        if (loader)
            requests.push(loader().then((module) => {
...
const sidebarFn = () => {
    ...
    if (!$toggleMenu || !$mobileSidebarMenus || !$menuMask)
        return;
    lifecycle.listen($toggleMenu, "click", (event) => {event.preventDefault();toggleMobileSidebar(true);});
    ...
    lifecycle.listen(window, "resize", () => {
        ...
        resizeFrame = requestAnimationFrame(() => {
            resizeFrame = null;
            if (Solitude.isHidden($toggleMenu) &&
...
const initAsideTagCloudOverflow = () => {
    ...
    const updateOverflowState = (tagCloud) => {
        tagCloud.classList.toggle("is-overflowing", !tagCloud.classList.contains("all-tags") &&
...
const percent = () => {
    ...
    document
        ...
        .forEach((item) => item.classList.toggle("hide", totalScrollableHeight - scrollPos < 100));
};
const showTodayCard = () => {
...
const initHomeCenter = () => {
    ...
    const profileCard = document.querySelector("#aside-content .card-info");
    let activeIndex = 0;
    let scrollFrame;
    ...
    const rgbToThemeHex = ([r, g, b]) => `#${[r, g, b]
        .map((value) => Math.floor(value * 0.8)
        .toString(16)
        .padStart(2, "0"))
        .join("")}`;
    ...
    const extractItemColor = (index, sourceImage) => {
        if (banners[index].dataset.color)
            return;
    ...
    const select = (index) => {
        ...
        if (profileTextColor) {
            profileCard?.style.setProperty("--profile-card-text-color", profileTextColor);
        }
    };
    ...
    banners.forEach((item, index) => {
        let pointerType = "";
        item.addEventListener("pointerdown", (event) => (pointerType = event.pointerType), true);
...
const actions = {
    ...
    runConfiguredAction(command) {
        ...
        if (call && typeof Solitude[call[1]] === "function") {
            ...
            if (navigation)
                Solitude.navigate(navigation[2]);
        }
    ...
    syncMusicState(isPlaying) {
        ...
        if ($music) {
            const capsule = $music;
            if (capsule.collapseTimer)
                window.clearTimeout(capsule.collapseTimer);
            ...
            else if (capsule.classList.contains("stretch") ||
    ...
    musicScrubberBind() {
        ...
        const restoreActualProgress = (aplayer) => {
            ...
            const currentTime = Math.min(duration, Math.max(0, Number(aplayer?.audio?.currentTime) || 0));
        ...
        document.addEventListener("pointermove", (event) => {
            ...
            const aplayer = getAPlayer();
            const currentSource = String(aplayer?.audio?.currentSrc || aplayer?.audio?.src || "");
        ...
        document.addEventListener("click", (event) => {
            if (!suppressClick || !event.target?.closest?.("#nav-music"))
                return;
            suppressClick = false;
            event.preventDefault();
    ...
    handleMusicClick(event) {
        if (event.target?.closest?.(".music-control-btn"))
            return;
        if (!this.musicPlaying)
            this.musicToggle();
    },
    ...
    onConsoleCardGroupClick(event) {
        if (event.target.closest?.(".console-card"))
            return;
        this.hideConsole();
    },
    onNavBlankClickCloseConsole(event) {
        if (!document.getElementById("console")?.classList.contains("show"))
            return;
        if (event.target.closest?.("a, button, .back-home-button, .menus_item, #page-name")) {
            return;
        }
        this.hideConsole();
    },
    refreshWaterFall() {
        ...
        const elements = allElements.filter((element) => element.dataset.solitudeWaterfall !== "true");
        if (!elements.length)
            return;
    ...
    addRuntime() {
        if (!Solitude.config.runtime)
            return;
        const runtime = Solitude.timeDiff(new Date(Solitude.config.runtime), new Date()) +
    ...
    toTalk(txt) {
        ...
        inputs.forEach((selector) => {
            ...
            if (el) {
                ...
                Solitude.scrollToDest(Solitude.getEleTop(document.getElementById("post-comment")), 300);
                el.focus();
    ...
    addPhotoFigcaption() {
        document
            .querySelectorAll(".article-container img:not(.gallery-item img, .inline-img)")
            .forEach((image) => {
            const captionText = image.getAttribute("alt");
            const figure = image.closest("figure");
            const hasFigureCaption = figure?.querySelector("figcaption");
            const hasGeneratedCaption = image.getAttribute("data-solitude-caption-ready") === "true" ||
                image.nextElementSibling?.matches(".img-alt");
            if (!captionText || hasFigureCaption || hasGeneratedCaption)
                return;
            image.setAttribute("data-solitude-caption-ready", "true");
            image.insertAdjacentHTML("afterend", `<div class="img-alt is-center">${Solitude.escapeHtml(captionText)}</div>`);
        });
    },
    scrollToComment: () => Solitude.scrollToDest(Solitude.getEleTop(document.getElementById("post-comment")), 300),
    setTimeState() {
    ...
    scrollCategoryBarToRight() {
        ...
        if (scrollBar) {
            const isScrollBarAtEnd = () => scrollBar.scrollLeft + scrollBar.clientWidth >=
    ...
    owoBig(owoSelector) {
        ...
        const hideOwoBig = (event) => {
            if (event.target.closest(owoSelector.item) &&
    ...
    homeTypeit() {
        if (typeof home_subtitle === "undefined")
            return;
...
class tabs {
    ...
    static expireAddListener() {
        ...
        if (!post_date)
            return;
        const ex = Math.ceil((new Date().getTime() -
...
const initGalleryMasonry = () => {
    ...
    galleries.forEach((gallery) => {
        const items = [...gallery.children].filter((item) => item.classList.contains("gallery-item"));
        if (!items.length)
            return;
        const layout = () => {
...
Solitude.refresh = async () => {
    ...
    const { runtime, lazyload, lightbox, randomlink, covercolor, expire } = Solitude.config;
