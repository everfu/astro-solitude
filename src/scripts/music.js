...
class MusicPlayer {
    ...
    waitForAPlayer() {
        ...
        this.loadingTimer = window.setInterval(() => {
            ...
            if (!aplayer || !root || !body || !cover || !list)
                return;
            clearInterval(this.loadingTimer);
            this.loadingTimer = null;
            if (loadingElement)
        ...
        if (currentLine && lyricContents) {
            ...
            if (index !== this.currentLyricIndex) {
                ...
                if (!this.isManualScrolling)
                    this.scrollLyricTo(currentLine, true);
            }
        }
    ...
    centerCurrentLyric(smooth = true) {
        const currentLine = this.getCurrentLyricLine();
        if (currentLine)
            this.scrollLyricTo(currentLine, smooth);
    }
    scrollLyricTo(line, smooth = true) {
        if (!this.lyricViewport || !line)
            return;
        ...
        const target = Math.min(maxScroll, Math.max(0, line.offsetTop - this.lyricViewport.clientHeight * 0.3 + line.offsetHeight / 2));
    ...
    handleLyricClick(event) {
        ...
        if (!line || !lyricContents || !this.aplayer)
            return;
    ...
    updateMediaSessionMetadata() {
        if (!("mediaSession" in navigator) || !("MediaMetadata" in window) || !this.aplayer)
            return;
    ...
    updateMediaSessionPosition() {
        if (!("mediaSession" in navigator) || !navigator.mediaSession.setPositionState || !this.audio)
            return;
        const position = Math.floor(this.audio.currentTime);
        const duration = this.audio.duration;
        if (position === this.lastMediaPosition || !Number.isFinite(duration) || duration <= 0)
            return;
