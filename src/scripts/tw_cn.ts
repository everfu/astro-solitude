...
const initializeTranslation = () => {
  if (!Solitude.config.translate) return;
  ...
  const {
    ...
    toSimplified: '简体中文',
    toTraditional: '繁體中文',
    switchedToSimplified: '已切换为简体中文',
    switchedToTraditional: '已切換為繁體中文',
  };
  const targetEncodingCookie = 'translate-chn-cht';
  let currentEncoding = defaultEncoding;
  let targetEncoding =
    Number(Solitude.saveToLocal.get(targetEncodingCookie)) || defaultEncoding;
  function setLang() {
    document.documentElement.lang = targetEncoding === 1 ? 'zh-TW' : 'zh-CN';
  }
  function translateText(txt: string | null) {
    if (!txt) return '';
    return currentEncoding === targetEncoding
      ? txt
      : currentEncoding === 1
        ? Simplized(txt)
        : Traditionalized(txt);
  }
  function translateBody(fobj: Node = document.body) {
    const objs = fobj?.childNodes || document.body.childNodes;
    objs.forEach((obj) => {
      if (obj instanceof HTMLElement && ['BR', 'HR'].includes(obj.tagName))
        return;
      if (obj instanceof HTMLElement && obj.title)
        obj.title = translateText(obj.title);
      if (obj instanceof HTMLImageElement && obj.alt)
      ...
      if (obj instanceof HTMLElement && obj.getAttribute('heotip')) {
        obj.setAttribute('heotip', translateText(obj.getAttribute('heotip')));
      }
      if (
        obj instanceof HTMLInputElement &&
        obj.value &&
        !['text', 'hidden'].includes(obj.type)
      ) {
  ...
  function updateButtonLabel(button: HTMLElement, label: string) {
  ...
  function translatePage(
    ...
    Solitude.snackbarShow(
      targetEncoding === 1 ? switchedToTraditional : switchedToSimplified,
    );
  ...
  function translateInitialization() {
    ...
    lifecycle.add(() => timers.forEach(clearTimeout));
    const btn_1 = document.getElementById('menu-translate');
    if (btn_1) {
      updateButtonLabel(
        btn_1,
        targetEncoding === 1 ? toSimplified : toTraditional,
      );
      if (currentEncoding !== targetEncoding) {
        setLang();
        timers.push(setTimeout(translateBody, translateDelay));
      }
      lifecycle.listen(btn_1, 'click', () => translatePage(btn_1));
    }
    const btn_2 = document.querySelector<HTMLElement>('.rs_hide .translate');
    if (btn_2) {
      if (btn_2.lastChild)
      ...
      lifecycle.listen(
        btn_2,
        'click',
        () => translatePage(btn_2, '简', '繁'),
        false,
      );
    }
  }
  translateInitialization();
  document.addEventListener('solitude:afterNavigate', translateInitialization);
};
