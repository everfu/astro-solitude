...
class AlgoliaSearch {
    ...
    init() {
        try {
            ...
            this.bindEvents();
            Solitude.listen(document,"keydown",(event)=>{if((event.ctrlKey||event.metaKey)&&event.key.toLowerCase()==="k"){event.preventDefault();this.openSearch();}if(event.key==="Escape")this.closeSearch();});
    ...
    bindSearchEvents() {
        ...
        if (this.elements.searchButton) {
            Solitude.addEventListenerPjax(this.elements.searchButton, "click", (event) => {event.preventDefault();this.openSearch();});
        }
        // 关闭按钮和遮罩
     ...
     */
    bindPjaxEvents() {
        window.addEventListener("solitude:afterNavigate", () => {
...
const initializeAlgoliaSearch = () => {
    Solitude.algoliaSearch?.destroy();Solitude.algoliaSearch = new AlgoliaSearch();Solitude.onPageCleanup(()=>Solitude.algoliaSearch?.destroy());
};
