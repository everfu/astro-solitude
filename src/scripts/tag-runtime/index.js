...
(() => {
    ...
    const initialConfig = {
        chartjs: window.Solitude?.config?.cdn?.chart || owner?.dataset.chartjs || 'https://cdn.jsdelivr.net/npm/chart.js@4.5.1/dist/chart.umd.js',
        abcjs: window.Solitude?.config?.cdn?.abcjs || owner?.dataset.abcjs || 'https://cdn.jsdelivr.net/npm/abcjs@6.6.4/dist/abcjs-basic-min.js'
    };
    ...
    const renderCharts = async () => {
        ...
        try {
            if (typeof window.Chart !== 'function')
                await loadScript(config.chartjs);
            ...
            containers.forEach((container, index) => {
                ...
                try {
                    ...
                    if (previous)
                        window.Chart.getChart(previous)?.destroy();
    ...
    const renderRepository = async (card) => {
        ...
        try {
            ...
            if (forksElement)
                forksElement.textContent = Number.isFinite(Number(forks)) ? Number(forks).toLocaleString() : '0';
    ...
    const renderTabs = () => {
        document.querySelectorAll('[data-solitude-tabs]:not([data-tabs-ready="true"])').forEach(container => {
            const panels = [...container.querySelectorAll(':scope > .tab-contents > .tab-item-content')];
            if (!panels.length)
                return;
            ...
            const activateTab = (activeIndex, moveFocus = false) => {
                nav.querySelectorAll('.tab').forEach((tab, index) => {
            ...
            panels.forEach((panel, index) => {
                ...
                if (!panel.id)
                    panel.id = `${container.id || 'tabs'}-panel-${index}`;
                button.type = 'button';
    ...
    const renderTypeit = () => {
        document.querySelectorAll('[data-typeit]:not([data-typeit-ready="true"])').forEach(element => {
            ...
            element.textContent = '';
            const instance=Reflect.construct(TypeIt, [element, { strings: [text], speed: Number(element.dataset.speed) || 80 }]);instance.go();window.Solitude?.onPageCleanup(()=>instance.destroy());
    ...
    document.addEventListener('error', event => {
        ...
        if (!(image instanceof HTMLImageElement) || !image.dataset.fallback || image.dataset.fallbackApplied)
            return;
        image.dataset.fallbackApplied = 'true';
        image.src = image.dataset.fallback;
    }, true);
    document.addEventListener('solitude:beforeNavigate', () => {destroyCharts();initialization=undefined;});
